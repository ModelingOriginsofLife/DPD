		...
		if (fabs(SLOPE) > 1.0) { \
			...
			for (y = (int)(PB); ( c > 0 ? y < floor(B) : y > (B) ); y += c) { \
				if (alpha < 255) \
					...
					SDL_putPixel(surf, x, y, clr); \
				if (flags & SDL_TG_ANTIALIAS) { \
					SDL_blendPixel(surf, x - 1, y, clr, (unsigned char)aa); \
					SDL_blendPixel(surf, x + 1, y, clr, (unsigned char)(alpha - aa)); \
					aa -= aastep; \
				} \
			} \
		} else { \
			if (alpha < 255) \
				SDL_blendPixel(surf, x, (int)(B), clr, alpha); \
			else \
				SDL_putPixel(surf, x, (int)(B), clr); \
			if (flags & SDL_TG_ANTIALIAS) { \
				aa = (B) - (int)(B); \
				aa *= alpha; \
				SDL_blendPixel(surf, x, (int)((B) + 1), clr, (unsigned char)aa); \
				SDL_blendPixel(surf, x, (int)((B) - 1), clr, (unsigned char)(alpha - aa)); \
			} \
		} \
	}
	...
	{ \
		if (fabs(SLOPE) > 2.0) { \
			...
			for (y = (int)((PB) + c); ( c > 0 ? y < floor(B) : y > (B) ); y += c) { \
				SDL_blendPixel(surf, x + aaoffset, y, clr, (unsigned char)aa); \
				aa += aastep; \
			} \
		} else { \
			if (B > B2) \
			...
			if (c < 0) \
				...
				aa *= alpha; \
			SDL_blendPixel(surf, x, (int)((B) + c), clr, (unsigned char)aa); \
		} \
	}
