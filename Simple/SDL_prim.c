		...
		if (fabs(SLOPE) > 1.0) { \
			...
			for (y = (PB); ( c > 0 ? y < floor(B) : y > (B) ); y += c) { \
				if (alpha < 255) \
					...
					SDL_putPixel(surf, x, y, clr); \
				if (flags & SDL_TG_ANTIALIAS) { \
					SDL_blendPixel(surf, x - 1, y, clr, aa); \
					SDL_blendPixel(surf, x + 1, y, clr, alpha - aa); \
					aa -= aastep; \
				} \
			} \
		} else { \
			if (alpha < 255) \
				SDL_blendPixel(surf, x, (B), clr, alpha); \
			else \
				SDL_putPixel(surf, x, (B), clr); \
			if (flags & SDL_TG_ANTIALIAS) { \
				aa = (B) - (int)(B); \
				aa *= alpha; \
				SDL_blendPixel(surf, x, (B) + 1, clr, aa); \
				SDL_blendPixel(surf, x, (B) - 1, clr, alpha - aa); \
			} \
		} \
	}
	...
	{ \
		if (fabs(SLOPE) > 2.0) { \
			...
			for (y = (PB) + c; ( c > 0 ? y < floor(B) : y > (B) ); y += c) { \
				SDL_blendPixel(surf, x + aaoffset, y, clr, aa); \
				aa += aastep; \
			} \
		} else { \
			if (B > B2) \
			...
			if (c < 0) \
				...
				aa *= alpha; \
			SDL_blendPixel(surf, x, (B) + c, clr, aa); \
		} \
	}
